...
import sailpoint.object.Link;
  ...
  List listOfAccountRequest=plan.getAccountRequests("AD B");
  // System.out.println("Account Request : "+listOfAccountRequest.size());

 
  for(AccountRequest accountReq:listOfAccountRequest) 
  {
     boolean set=false;
    log.error("accountReq---------------"+accountReq.toXml());
    List  attributeReq = accountReq.getAttributeRequests();
    //System.out.println("Access to number of Entitlements :  " + attributeReq.size());

    for(AttributeRequest attrReq:attributeReq)
    {
      log.error("accountReq---------"+accountReq);
      String en= attrReq.getValue();
      System.out.println("Value : " + en);
      String target ="CN=Admins,OU=Groups,OU=Demo,DC=Eshiam,DC=com";
    ...
    { 
      ...
      log.error("attributeRequest---------"+attributeRequest.toXml());
      attributeRequest.setOp(ProvisioningPlan.Operation.Add);
      attributeRequest.setName("memberOf");
      attributeRequest.setDisplayValue("ESHIAM\'Admins");
      attributeRequest.setValue("CN=Admins,OU=Groups,OU=Demo,DC=Eshiam,DC=com");
      attributeRequest.setAssignment(true);
      log.error("attributeRequest---------"+attributeRequest.toXml());
      accountReq.add(attributeRequest);
    }
  }
  System.out.println("AfterModification"+plan.toXml());